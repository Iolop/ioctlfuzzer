...
char *GetNormalizedDriverFilePath(char *lpszPath)
{
    ...
    if (lpszPath_l)
    {
        ...
        else if (!strncmp(lpszPath_l, "\\systemroot\\", 12))
        {
            ...
            GetEnvironmentVariable("SystemRoot", szPath, MAX_PATH - 1);
    ...
    if (s)
    {
        ...
        char *lpszExp = NULL;
        DWORD ExpLen = ExpandEnvironmentStrings(s, lpszExp, 0);
        if (ExpLen > 0)
        {
            ExpLen += 2;
            if (lpszExp = (char *)M_ALLOC(ExpLen))
            {
                if (ExpandEnvironmentStrings(s, lpszExp, ExpLen) > 0)
                {
                    M_FREE(s);
                    s = lpszExp;
                }
...
BOOL GetDescrAndCompanyInfo(char *lpszFilePath, char **lpszDescr, char **lpszCompany)
{
    ...
    if (dwSize > 0)
    {
        ...
        if (pInfo)
        {
            ...
            if (GetFileVersionInfo(lpszFilePath, dwHandle, dwSize, pInfo))
            {
                UINT uValueSize = 0;
                struct LANG_INFO 
                {
...
DWORD ParseIoctlsLog(char *lpszIoctlsLogPath)
{
    ...
    HANDLE hFile = CreateFile(lpszIoctlsLogPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    if (hFile == INVALID_HANDLE_VALUE)
    {
